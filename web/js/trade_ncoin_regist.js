...
function setting_progress() {
	...
	setting_html += `<div class="message_area"><p>수량 및 가격을 설정하십시오.</p></div>`;
...
function finish_progress() {
	...
	if (reg_price < 1000) {
		popupShow('최소 등록 금액은 1,000코인 입니다.', '<span class="type2"><a href="javascript:popupClose();" class="close">닫기</a></span>', null);
		return;
	}
	if (reg_price % 1000 != 0) {
		popupShow('1,000코인 단위로 등록할 수 있습니다.', '<span class="type2"><a href="javascript:popupClose();" class="close">닫기</a></span>', null);
		return;
	}
	...
	finish_html += `<div class="coin_area txt">판매가격: <span class="ncoin">${reg_price_val}</span></div>`;
