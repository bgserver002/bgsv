	...
	setInstance() {
		...
		if (_.option.data) {
			let tmpl_item = _.option.data.map( (item, i) => {
				...
				return `<li class="${item.visible}"><a href="${_url}" target="${item.target}"><span class="icon" style="background-image:url(${item.image})"></span><span class="name">${item.name}</span></a></li>`;
			}).join('');
...
class PromotionBanner {
	...
	init() {
		...
		_.is_admin = account && account.gm;
		let tmpl_bannerItem = _.option.data.map( (item, i) => {
			return `<li data-title="${item.title}" data-desc="${item.desc}" data-date="${item.date}" data-url="${item.url}" data-target="${item.target}" data-banner="${item.banner}" data-thumbnail="${item.thumbnail}" data-flag="${item.flagTitle}">` +
				`<div class="bg-mobile" data-lazy="${item.banner}"></div>` +
				`<div class="bg" data-lazy="${item.banner}"></div>` +
				`<div class="article">` +
					`<a href="${item.url}" class="${item.target}">` +
						`${item.flag ?  `<span class="type is-type">${item.flagTitle}</span>` : `<span class="type"></span>`}` +
						`<strong class="title">${item.title}</strong>` +
						`<span class="desc">${item.desc}</span>` +
						`<span class="date">${item.date}</span>` +
					`</a>` +
				`</div>` +
				(_.is_admin ? `<div class="promotion_admin_util"><button class="promotion_modify_btn" data-id="${item.id}">수정</button><button class="promotion_del_btn" data-id="${item.id}">삭제</button><button class="promotion_add_btn">추가</button></div>` : ``) +
			`</li>`;
		}).join('');
	...
	addEventAdmin() {
		...
		_.promotion_admin_sync = false;

		// 추가 버튼 클릭 이벤트
		$('.promotion_admin_util .promotion_add_btn').on('click', (e) => {
			_.promotionInsert();
		});

		// 제거 버튼 클릭 이벤트
		$('.promotion_admin_util .promotion_del_btn').on('click', function(e) {
			_.promotionDelete($(this).attr('data-id'));
		});

		// 수정 버튼 클릭 이벤트
		$('.promotion_admin_util .promotion_modify_btn').on('click', function(e) {
			_.promotionModify($(this).attr('data-id'), this);
		});

	}

	promotionUtilForm(type) {
		const _ = this;
		...
		`<div class="promotionUtilForm">` +
			`<div class="promotionUtilForm_container">` +
				`<form name="promoForm" onsubmit="return false;">` +
					...
					`<div class="util_items"><label>제목</label><input type="text" name="promoTitle" id="promoTitle" placeholder="필수항목" /></div>` +
					`<div class="util_items"><label>내용</label><input type="text" name="promoContent" id="promoContent" placeholder="필수항목" /></div>` +
					`<div class="util_items"><label>날짜</label><input type="text" name="promoDate" id="promoDate" placeholder="생략가능" /></div>` +
					`<div class="util_items"><label>링크</label><input type="text" name="promoLink" id="promoLink" placeholder="필수항목(이동 경로)" /></div>` +
					`<div class="util_items"><label>대상</label><select name="promoTarget" id="promoTarget"><option value="self" selected>self</option><option value="blank">blank</option><option value="outlink">outlink</option></select></div>` +
					`<div class="util_items filebox"><label>이미지</label><i><input class="upload-name" id="promoImgStr" name="promoImgStr" placeholder="필수항목(이미지 파일)" readonly><label class="file-label" for="promoImg">파일찾기</label><input type="file" name="promoImg" id="promoImg" class="file"></i></div>` +
					`<div class="util_items filebox"><label>썸네일</label><i><input class="upload-name" id="promoSubImgStr" name="promoSubImgStr" placeholder="필수항목(이미지 파일)" readonly><label class="file-label" for="promoSubImg">파일찾기</label><input type="file" name="promoSubImg" id="promoSubImg" class="file"></i></div>` +
					`<div class="util_items"><label>강조</label><input type="text" name="promoFlagTitle" id="promoFlagTitle" placeholder="생략가능" /></div>` +
					`${type == 'update' ? `<div class="btns"><button class="promotion_cancel_btn">취소</button><button class="promotion_modify_btn">수정</button></div>` : `<div class="btns"><button class="promotion_cancel_btn">취소</button><button class="promotion_insert_btn">추가</button></div>`}` +
				`</form>` +
			`</div>` +
		`</div>`;

		// 폼 출력
		$('#container').append(tpl_form);
		$('body').addClass('ncc-dimmed');

		// 추가 버튼 클릭 이벤트
		$('.promotionUtilForm .promotion_insert_btn').on('click', (e) => {
			_.promotionAdd();
			return false;
		});

		// 수정 버튼 클릭 이벤트
		$('.promotionUtilForm .promotion_modify_btn').on('click', (e) => {
			_.promotionUpdate();
			return false;
		});

		// 취소 버튼 클릭 이벤트
		$('.promotionUtilForm .promotion_cancel_btn').on('click', (e) => {
			_.promotionCancel();
			return false;
		});

		// 이미지 변경 이벤트
		$('.promotionUtilForm #promoImg').on('change', function(e) {
			_.promoFileChange(this);
			return false;
		});

		// 보조 이미지 변경 이벤트
		$('.promotionUtilForm #promoSubImg').on('change', function(e) {
			_.promoFileChange(this);
			return false;
		});
	}
	...
	promotionInsert(id, obj) {
		const _ = this;

		const promoMax	= 16;// 최대 저장 가능 개수
	...
	promotionAdd() {
		...
		if (_.promotion_admin_sync) {
			return;
		}
		_.promotion_admin_sync = true;
		...
		getDatasWithOption([{
			...
			_.promotion_admin_sync = false;
			throw new Error("promotionAdd from main.js Error");
		});
	}

	promotionModify(id, obj) {
		const _ = this;

		_.promotionUtilForm('update');
		...
		$('.promotionUtilForm #promoContent').val(promoTarget.attr('data-desc'));
		$('.promotionUtilForm #promoDate').val(promoTarget.attr('data-date'));
		$('.promotionUtilForm #promoLink').val(promoTarget.attr('data-url'));
		$(".promotionUtilForm select[name=promoTarget] option[value=" + promoTarget.attr('data-target') + "]").prop('selected', true);
		$('.promotionUtilForm #promoImg').closest('.filebox').find(".upload-name").val(promoTarget.attr('data-banner'));
		$('.promotionUtilForm #promoSubImg').closest('.filebox').find(".upload-name").val(promoTarget.attr('data-thumbnail'));
		$('.promotionUtilForm #promoFlagTitle').val(promoTarget.attr('data-flag'));
	}
	...
	promotionUpdate() {
		...
		if ((mainPromoImg && mainPromoImg.size > limitSize) || (subPromoImg && subPromoImg.size > limitSize)) {
			alert('이미지는 1MB이하로 등록하십시오.');
			return;
		}
		...
		getDatasWithOption([{
			...
			_.promotion_admin_sync = false;
			throw new Error("promotionUpdate from main.js Error");
		});
	}
	...
	promotionDelete(id) {
		...
		getDatasWithOption([{
			...
			_.promotion_admin_sync = false;
			throw new Error("promotionDelete from main.js Error");
		});
	}
...
class NewsContent {
	...
	setInstance() {
		...
		let tmpl_item = _.option.data.items.map( (item, i) => {
			let is_new_flag = nowTime - moment(new Date(item.date)).format('YYYYMMDD') == 0;
			return `<li class="is-new-${is_new_flag}"><a href="javascript:urlTypeform('0', '${item.rownum}', 'post', '/notice/view');">${item.title}</a>${is_new_flag ? `<span class="icon-new"></span>`:``}</li>`;
		}).join('');

		let tmpl = 
		`<header><h2><a href="${_.option.data.headerUrl}">${_.option.data.header}</a></h2></header>` +
		`<article>` +
			`<a href="${_.option.data.url}">` +
				`<dl><dd class="thumb" style="background-image:url('${_.option.data.thumbnail}');"><img src="${_.option.data.thumbnail}" class="thumb"></dd>` +
				`<dt>${_.option.data.title}</dt></dl>` +
			`</a>` +
			`<ul>${tmpl_item}</ul>` +
		`</article>`;
		_.option.wrap.append(tmpl);
...
class HotIssue {
	...
	setInstance() {
		...
		let tmpl_item = _.option.data.map((item, i) => {
			return `<li>${item[0] == '-' ? `<a href="javascript:;">` : `<a href="/search?query=${item[0]}">`}${item[0]}</a><span class="ui-rank ${item[1]}">${item[2]}</span></li>`;
		}).join('');

		let tmpl = `<header><h2>핫이슈</h2><span class="btn-toggle"></span></header><article><ol class="keyword">${tmpl_item}</ol></article>`;
		_.option.wrap.append(tmpl);
	...
	addEvents() {
		// 모바일 축소된 핫이슈 전체 보기 버튼 이벤트
		$('.section-hotissue span.btn-toggle').on('click', function(){
			$('.section-hotissue').toggleClass('on');
		});
	}
...
class TopCard {
	...
	setInstance() {
		...
		// 3, 4번 카드
		let tmpl = _.option.data.map( (item, i) => {
			return `<section class="section-powerbook section-${item.contentsId}">` +
				`<header><h2><a href="${item.headerUrl}">${item.header}</a></h2></header>` +
				`<article>` +
					`<a href="${item.url}">` +
						`<dl><dd class="thumb" style="background-image:url('${item.thumbnail}');"><img src="${item.thumbnail}" class="thumb"></dd>` +
							`<dt>${item.title}</dt><dd class="desc" style="max-height: 24px; overflow: hidden;">${item.desc}</dd>` +
						`</dl>` +
					`</a>` +
				`</article>` +
			`</section>`;
...
class SectionContent {
	...
	setInstance() {
		...
		function powerbook_tmpl_item (item) {
			return `<div class="slick-item"><a href="/powerbook/search?searchType=4&query=${item.name}"><dl><dd class="thumb"><span class="flag 파워북">파워북</span><img src="${item.info.mainImg}" class="thumb"></dd><dt><strong>파워북</strong></dt><dd class="desc">${item.name}</dd></dl></a></div>`;
		}
		...
		let tmpl = _.option.data.map( (section, i) => {
			...
			//if (section.contentsType === 'type4') slick3List.push(section.contentsId);
			if (section.contentsType === 'type6') slick6List.push(section.contentsId);

			if (section.contentsType === 'type4') {
				return `<section class="section-content-${section.contentsType} true" data-order-id="${section.orderId}">` +
					`<div class="bundle">` +
						`<header><h2>${section.header}</h2></header>` +
						`<article id="${section.contentsId}">${section.items.map( (item, index) => { return section.boad_type === 'FREE' ? board_tmpl_item(item, index) : contents_tmpl_item(item, index);  }).join('')}</article>` +
					`</div>` +
				`</section>`;
			} else {
				return `<section class="section-content-${section.contentsType} true" data-order-id="${section.orderId}">` +
					`<header><h2>${section.header}</h2></header>` +
					`<article id="${section.contentsId}">${section.items.map( (item, index) => { return powerbook_tmpl_item(item); }).join('')}</article>` +
				`</section>`;
			}
		}).join('');
...
class NshopContent {
	...
	setInstance() {
		...
		if (!_.option.data) {
			_.option.wrap.append('<header><h2><a href="/goods">리니지N샵</a></h2></header><div class="mainContent_empty"><p>등록된 상품이 없습니다.</p></div>');
			return;
		}
		...
		let tmpl_item = _.option.data.map( (item, i) => {
			return `<section class="nshop__item">` +
				`<a href="javascript:urlform('${item[0].id}', 'post', '/goods/view');">` +
					`<span class="flag flag-none"></span>` +
					`<dl>` +
						`<dd class="thumb"><img src="${item[0].icon_image}" alert="${item[0].title}"></dd><dt>${item[0].title}</dt>` +
						`<dd class="price">${item[0].price_type === 'NCOIN' ? `<i class="price_ncoin"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path fill="#F16126" d="M18.965 12.51l-6.455 6.455a3.56 3.56 0 0 1-5.02 0L1.035 12.51a3.56 3.56 0 0 1 0-5.02L7.49 1.035a3.56 3.56 0 0 1 5.02 0l6.455 6.455a3.56 3.56 0 0 1 0 5.02"></path><path fill="#FFF" d="M12.5 6.989s-2.374-.279-2.809 0c0 0-1.02.471-1.02 1.957v2.108c-.018 1.486 1.02 1.957 1.02 1.957.435.279 2.809 0 2.809 0v1.829c-.877.277-2.68.26-2.68.26-3.038-.002-3.32-3.394-3.32-3.394V8.294S6.782 4.902 9.82 4.9c0 0 1.803-.017 2.68.261V6.99z"></path></svg></i>` : `<i class="price_npoint"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path fill="#F16126" d="M18.965 12.51l-6.455 6.455a3.56 3.56 0 0 1-5.02 0L1.035 12.51a3.56 3.56 0 0 1 0-5.02L7.49 1.035a3.56 3.56 0 0 1 5.02 0l6.455 6.455a3.56 3.56 0 0 1 0 5.02"></path><path fill="#FFF" d="M12.5 6.989s-2.374-.279-2.809 0c0 0-1.02.471-1.02 1.957v2.108c-.018 1.486 1.02 1.957 1.02 1.957.435.279 2.809 0 2.809 0v1.829c-.877.277-2.68.26-2.68.26-3.038-.002-3.32-3.394-3.32-3.394V8.294S6.782 4.902 9.82 4.9c0 0 1.803-.017 2.68.261V6.99z"></path></svg></i>`}<strong>${item[0].price_comma}</strong></dd>` +
					`</dl>` +
				`</a>` +
			`</section>`;
		}).join('');

		let tmpl = `<header><h2><a href="/goods">리니지N샵</a></h2></header><article id="nshopRoll"><div class="slick-item">${tmpl_item}</div></article>`;
...
(function ($, global) {
	...
	nc.lineage.main = function (options) {
		...
		contiGroupData.topcard = [];

		$(options.promotionBanner.wrap).append(getLoadingBarTpl());
		$(options.sectionContent.wrap).append(getLoadingBarTpl());
		...
		function init() {
			...
			$.each(index_data, function(key, element) {
				...
				case 'PROMOTIONS':
					...
					break;
				case 'NOTICE':
					const main_news = element ? element.shift() : null;// 1번째 데이터 꺼내기
					contiGroupData.news = {
						items : element,
						header : '공지사항',
						headerUrl : '/notice',
				...
				case 'EVENT':
					...
					break;
				case 'KEYWORD':
					contiGroupData.hotissue = element;
					break;
				case 'SHORTCUT':
					contiGroupData.shortcut = element;
					break;
				case 'GUIDE':
					contiGroupData.section[0] = {
						contentsId : 'lineageworld',
						contentsType : 'type6',
						header : '파워북',
						items : element,
						orderId : 0
					...
					break;
				case 'COMMUNITY':
					contiGroupData.section[1] =  {
						contentsId : 'board_user',
						contentsType : 'type4',
						header : 'COMMUNITY: 서버 계시판!',
						items : element,
						orderId : 1,
						boad_type : 'FREE'
					};
					break;
				case 'CONTENT':
					contiGroupData.section[2] =  {
						contentsId : 'content_user',
						contentsType : 'type4',
						header : 'CONTENTS: 컨텐츠 공모!',
						items : element,
						orderId : 2,
						boad_type : 'CONTENTS'
					};
	...
	// 게임 이용 등급 버튼 클릭 이벤트
	let grade_layer = $('#FooterGradeLayer');
	$('body').mouseup(function (e) {
		if (!grade_layer.has(e.target).length && grade_layer.hasClass('is-show-footer')) {
			grade_layer.removeClass('is-show-footer');
		}
	});
	$('#grade-layer-item').on('click', function(e) {
		if (!grade_layer.hasClass('is-show-footer')) {
			grade_layer.addClass('is-show-footer');
		} else {
			grade_layer.removeClass('is-show-footer');
		}
	});
	...
	// 인게임 브라우저 내장 함수 보기
	if (account && account.ingame && account.gm) {
		$('.section-etcservice').prepend(`<a class="l1_function_btn">함수 보기</a>`);
		$('.section-etcservice .l1_function_btn').on('click', function(e) {
			let tpl_property = Object.getOwnPropertyNames(L1).map( (item, i) => {
				return `<li><strong>${item}</strong></li>`;
			}).join('');
			$('body').prepend(`<div id="l1_functions" style="position: fixed; left: 35%; top: 10%; z-index: 111111111; padding: 10px; background: #000; color: #fff; max-height: 400px;overflow: auto;"><ul>${tpl_property}</ul><a href="javascript:$('#l1_functions').remove();" style="margin: auto; display: table; padding: 10px; background: cadetblue; color: #fff; font-weight: 600;">닫기</a></div>`);
			eventStop(e);
		});
	}
}(jQuery, window));